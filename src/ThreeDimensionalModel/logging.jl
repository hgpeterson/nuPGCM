...
function save_setup(m::ModelSetup3D, save_file)
    ...
    write(file, "HHM", m.evolution.HHM)
    ...
    # write(file, "Ax_HHM_SD", Array(m.evolution.Ax_HHM_SD))
    # write(file, "Ay_HHM_SD", Array(m.evolution.Ay_HHM_SD))
    ...
    log_params(ofile, @sprintf("δ₀  = %1.1e", m.params.δ₀))
    log_params(ofile, "\nResolution:")
...
function load_setup_3D(filename)
    ...
    # Ax2 = CuArray(read(file, "Ax2"))
    # Ay2 = CuArray(read(file, "Ay2"))
    # Ax_HHM_SD = CuArray(read(file, "Ax_HHM_SD"))
    # Ay_HHM_SD = CuArray(read(file, "Ay_HHM_SD"))
    # CUDA.memory_status()
    advection = read(file, "advection")
    # evolution = EvolutionComponents(HM, HHM, K_cols, Ax1, Ay1, Ax2, Ay2, Ax_HHM_SD, Ay_HHM_SD, advection)
    evolution = EvolutionComponents(HM, HHM, K_cols, advection)
