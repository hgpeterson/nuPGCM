...
function save_setup(m::ModelSetup3D, save_file)
    ...
    # InversionComponents
    I, J, V = findnz(m.inversion.Dx)
    write(file, "Dx_I", I)
    write(file, "Dx_J", J)
    write(file, "Dx_V", V)
    I, J, V = findnz(m.inversion.Dy)
    write(file, "Dy_I", I)
    write(file, "Dy_J", J)
    write(file, "Dy_V", V)
    ...
    # EvolutionComponents
    I, J, V = findnz(m.evolution.HM)
    write(file, "HM_I", I)
    write(file, "HM_J", J)
    write(file, "HM_V", V)
    I, J, V = findnz(m.evolution.K_stab)
    write(file, "K_stab_I", I)
    write(file, "K_stab_J", J)
    write(file, "K_stab_V", V)
...
function load_setup_3D(filename)
    ...
    V = read(file, "HM_V")
    HM = sparse(I, J, V)
    I = read(file, "K_stab_I")
    J = read(file, "K_stab_J")
    V = read(file, "K_stab_V")
    K_stab = sparse(I, J, V)
    ...
    evolution = EvolutionComponents(HM, K_stab, LHS_diffs, RHS_diffs, Ax, Ay, advection)
