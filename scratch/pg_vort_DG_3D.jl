...
struct Column{FM<:AbstractMatrix, IM<:AbstractMatrix, IV<:AbstractVector, IN<:Integer} <: nuPGCM.Grid{FM, IM, IV, IN}
    ...
    sfc::IV # integer vector

    # number of surface nodes
    nsfc::IN

    # bottom node indices
    bot::IV # integer vector
...
function gen_mesh(ifile; order)
    ...
    cols = Vector{Column}(undef, nt_sfc)
    ...
    for k=1:nt_sfc
        ...
        p_col = p[nodes_col, :]  
        e_sfc = [1, lens[1]+1, lens[1]+lens[2]+1]
        e_bot = [lens[1], lens[1]+lens[2], lens[1]+lens[2]+lens[3]]
        ...
        # compute Jacobians
        J = Jacobians(p_col, t_col)

        # add midpoints if second order
        if order == 2
            emap, edges, bndix = all_edges(t_col)

            # add midpoints
            np0 = size(p_col, 1)
            midpts = 1/2*reshape(p_col[edges[:, 1], :] + p_col[edges[:, 2], :], (size(edges, 1), :))
            p_col = [p_col; midpts]

            # map to triangle data structure
            t_col = hcat(t_col, np0 .+ emap)

            # add points that were on the sfc or bdy
            e_sfc0 = copy(e_sfc)
            e_bot0 = copy(e_bot)
            for i ∈ axes(edges, 1)
                if edges[i, 1] ∈ e_sfc0 && edges[i, 2] ∈ e_sfc0
                    push!(e_sfc, np0 + i)
                end
                if edges[i, 1] ∈ e_bot0 && edges[i, 2] ∈ e_bot0
                    push!(e_bot, np0 + i)
                end
            end
        end

        # remove from e_bot if in e_sfc
        e_bot = e_bot[findall(i -> e_bot[i] ∉ e_sfc, 1:size(e_bot, 1))]

        # save column data
        cols[k] = Column(order, size(p_col, 2), sf, sfi, J, p_col, size(p_col, 1), t_col, size(t_col, 1), size(t_col, 2), e_sfc, size(e_sfc, 1), e_bot, size(e_bot, 1))
    end
...
function plot_1D(col, sol)
    ...
    x = 1/size(col.sfc,1)*sum(col.p[col.sfc[:], 1])
    y = 1/size(col.sfc,1)*sum(col.p[col.sfc[:], 2])
...
function plot_3D()
    ...
    nsfc = sum(col.nsfc for col ∈ cols) 
    nbot = sum(col.nbot for col ∈ cols) 
    ...
    for i ∈ eachindex(cols)
        ...
        sfc[i_sfc+1:i_sfc+col.nsfc] = i_p .+ col.sfc
        bot[i_bot+1:i_bot+col.nbot] = i_p .+ col.bot
