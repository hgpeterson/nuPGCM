@testset "Bowl Dirichlet b" begin
    h  = 0.1
    α  = 0.5
    ...
    b_surface(x) = x[2]
    forcings = Forcings(1.0, x->1.0, x->1.0, x->0.0, x->0.0,
                        SurfaceDirichletBC(b_surface))
    ...
    datafile = joinpath(@__DIR__, "data/bowl_dirichlet.jld2")
    if !isfile(datafile)
        @warn "Reference not found, saving bowl Dirichlet state..."
