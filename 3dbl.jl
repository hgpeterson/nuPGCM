...
function analyticalBL(σ, f, Hx, Hy, H, ν, κ, dbdξ_I, dbdη_I, uξ_I, uη_I)
    ...
    c1 = (uξ_I*(q^2 - p^2) - uη_I*δ^2*(p^2 + q^2)^2/2)/(2*p*q)
    c2 = (uξ_I + uη_I*δ^2*(p^2 - q^2)/2)/(δ^2*p*q)
    # c3 = f/Hx * (uξ_I*δ^2*(p^2 - q^2) + uη_I*(δ^4*(p^2 + q^2)^2/4 - 1))
    # c4 = f/(Hx*δ^2*p*q) * (uξ_I*(1 + δ^4/4*(p^4 - 6*p^2*q^2 + q^4)) + uη_I*(δ^2/2*(p^2 - q^2)*(1 + δ^4/4*(p^2 + q^2)^2)))
    c3 = -f/Hx * (uη_I +  uξ_I*δ^2*(q^2 - p^2)/2 + c1*δ^2*p*q)
    c4 = f/Hx * (-uξ_I*δ^2*p*q + δ^2*c1*(q^2 - p^2)/2 + c2)

    uξ_B = @. exp(-q*H*(σ + 1))*(-uξ_I*cos(p*H*(σ + 1)) + c1*sin(p*H*(σ + 1)))
    uη_B = @. exp(-q*H*(σ + 1))*(-uη_I*cos(p*H*(σ + 1)) + c2*sin(p*H*(σ + 1)))
    b_B  = @. exp(-q*H*(σ + 1))*(c3*cos(p*H*(σ + 1)) + c4*sin(p*H*(σ + 1)))
    # b_B = zeros(size(uξ_B))
    # dσ = σ[2] - σ[1]
