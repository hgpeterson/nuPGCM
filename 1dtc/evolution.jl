...
If sol = (û, v̂, b, P_x) then we compute `A` and `diffVec` such that sol_t = A*sol + diffVec
...
function getMatrices()
    ...
    for j=2:nẑ-1
        ...
        ν_ẑ = sum(fd_ẑ.*ν[j-1:j+1])
        κ_ẑ = sum(fd_ẑ.*κ[j-1:j+1])

        # dẑẑ stencil
        fd_ẑẑ = mkfdstencil(ẑ[j-1:j+1], ẑ[j], 2)

        # 1st eqtn: û_t = f*v̂*cos(θ) - P_x*cos(θ) + b*sin(θ) + (ν*û_ẑ)_ẑ
        row = umap[1, j]
        # first term
        ...
        # fourth term: dẑ(ν*dẑ(û))) = dẑ(ν)*dẑ(û) + ν*dẑẑ(û)
        push!(A, (row, umap[1, j-1], ν_ẑ*fd_ẑ[1] + ν[j]*fd_ẑẑ[1]))
        push!(A, (row, umap[1, j],   ν_ẑ*fd_ẑ[2] + ν[j]*fd_ẑẑ[2]))
        push!(A, (row, umap[1, j+1], ν_ẑ*fd_ẑ[3] + ν[j]*fd_ẑẑ[3]))

        # 2nd eqtn: v̂_t = -f*û*cos(θ) + (ν*v̂_ẑ)_ẑ
        row = umap[2, j]
        # first term:
        push!(A, (row, umap[1, j], -f*cos(θ)))
        # second term: dẑ(ν*dẑ(v̂))) = dẑ(ν)*dẑ(v̂) + ν*dẑẑ(v̂)
        push!(A, (row, umap[2, j-1], ν_ẑ*fd_ẑ[1] + ν[j]*fd_ẑẑ[1]))
        push!(A, (row, umap[2, j],   ν_ẑ*fd_ẑ[2] + ν[j]*fd_ẑẑ[2]))
        push!(A, (row, umap[2, j+1], ν_ẑ*fd_ẑ[3] + ν[j]*fd_ẑẑ[3]))

        # 3rd eqtn: b_t = -N^2*û*sin(θ) + [κ*(N^2*cos(θ) + b_ẑ)]_ẑ
        row = umap[3, j]
        # first term
    ...
    if transportConstraint
        # transport-constrained 1D: P_x such that ∫ û dẑ = U₀
        for j=1:nẑ-1
            # trapeẑoidal rule: (û_j+1 + û_j)/2 * Δẑ_j
...
function evolve(tFinal)
    ...
    Px = sol[nPts]
    iSave = 0
    saveCheckpoint1DTC(b, û, v̂, Px, t, iSave)
    iSave += 1
